...
{
    /// <summary>
    /// Точка входа для VST плагина PADGenerator
    /// </summary>
    public sealed class PluginCommandStub : StdPluginCommandStub
    {
        ...
        private static Assembly OnAssemblyResolve(object sender, ResolveEventArgs args)
        {
            try
            {
                ...
                foreach (string dllName in embeddedDlls)
                {
                    ...
                    {
                        // Пытаемся загрузить из встроенных ресурсов (они встроены как PADGeneratorVST.bin.Release.{dllName}.dll)
                        string resourceName = $"PADGeneratorVST.bin.Release.{dllName}.dll";
        ...
        {
            return new PADGeneratorVstPlugin();
        }
    }
}
