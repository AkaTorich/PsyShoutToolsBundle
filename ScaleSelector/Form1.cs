...
namespace ScaleSelector
{
    ...
    {
        // Данные (static readonly для оптимизации)
        ...
        private static readonly Dictionary<string, Dictionary<string, List<int>>> BigScalePatterns = new Dictionary<string, Dictionary<string, List<int>>>
{
    ...
    "Микротональные лады", new Dictionary<string, List<int>>
    {
        ...
        public ScaleSelectorForm()
        {
            InitializeComponent();
            PopulateComboBoxes();
            // Добавляем обработчик события изменения категории
            comboBoxCategory.SelectedIndexChanged += ComboBoxCategory_SelectedIndexChanged;
        }

        private void PopulateComboBoxes()
        {
            // Заполняем comboBoxCategory
            ...
            // Заполняем comboBoxTonic без сортировки, так как порядок хроматический
            ...
            // comboBoxScale заполняется в ComboBoxCategory_SelectedIndexChanged
        }

        private void ComboBoxCategory_SelectedIndexChanged(object sender, EventArgs e)
        {
            // Очищаем comboBoxScale и заполняем его гаммами из выбранной категории
        ...
        {
            if (comboBoxCategory.SelectedIndex == -1 || comboBoxScale.SelectedIndex == -1 || comboBoxTonic.SelectedIndex == -1)
            {
                MessageBox.Show("Пожалуйста, выберите категорию, гамму и тонику.", "Предупреждение", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (BigScalePatterns.TryGetValue(selectedCategory, out var scales) && scales.TryGetValue(selectedScale, out List<int> pattern))
            {
                List<string> notes = CalculateScale(selectedTonic, pattern);
                if (notes.Count > 0)
                {
                    richTextBoxNotes.Clear();
                    richTextBoxNotes.AppendText($"Гамма: {selectedScale} ({notes.Count} нот)\n");
