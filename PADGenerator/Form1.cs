            ...
            var bigScalePatterns = new Dictionary<string, Dictionary<string, List<int>>>
{
    ...
    "Микротональные лады", new Dictionary<string, List<int>>
    {
        ...
        private void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Введите корректное количество уникальных аккордов (число больше 0).", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (!int.TryParse(txtProgressionLength.Text, out numberOfChords) || numberOfChords <= 0)
            {
                MessageBox.Show("Введите корректное общее количество аккордов в прогрессии (число больше 0).", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
