...
namespace BASSGenerator
{
    ...
    public class NoteDisplayControl : Control
    {
        ...
        // Цвет нот - изменен на DeepSkyBlue
        private readonly Color _noteColor = Color.DeepSkyBlue;
        ...
        public void SetNotesWithDurations(List<int> notes, List<int> durations)
        {
            // Очищаем существующие ноты
            _notes.Clear(); // Было this.notes.Clear();
            ...
            for (int i = 0; i < notes.Count; i++)
            {
                ...
                // Если встретился маркер аккорда, обрабатываем его
                if (note == -1)
                {
                    // Это маркер для отображения нескольких нот в аккорде
                    // Продолжаем текущую позицию без изменений
                    continue;
                }

                int duration = (i < durations?.Count) ? durations[i] : 1;

                // Добавляем ноту
                _notes.Add(new NoteInfo(note, currentPosition, duration)); // Было this.notes.Add(note);

                // Увеличиваем позицию только если это не часть аккорда
                if (i + 1 < notes.Count && notes[i + 1] != -1)
                {
                ...
                for (int i = 0; i < visibleNotesCount && highestVisibleNote - i >= lowestVisibleNote; i++)
                {
                    ...
                    using (SolidBrush keyBrush = new SolidBrush(isBlackKey ? _blackKeyColor : _whiteKeyColor))
                    {
                        ...
                        if (isCNote)
                        {
                            ...
                            using (SolidBrush textBrush = new SolidBrush(Color.DeepSkyBlue)) // DeepSkyBlue цвет для C
                            {
            ...
            using (Pen noteBorderPen = new Pen(Color.White, 1)) // Белая рамка для лучшей видимости
            {
                foreach (var note in _notes)
                {
                        ...
                        (position + duration) >= firstVisiblePos && position <= lastVisiblePos)
                    {
                        ...
                        // Для всех нот используем DeepSkyBlue цвет, но для нот C делаем немного темнее
                        using (SolidBrush noteBrush = new SolidBrush(isCNote ?
                            Color.FromArgb(0, 150, 200) : _noteColor))
                        {
                ...
                foreach (var note in _notes)
                {
                    ...
                    if (noteX >= -NOTE_RADIUS * 2 && noteX <= clientRect.Width + NOTE_RADIUS * 2)
                    {
                        ...
                        if (notePositions.ContainsKey(normalizedNote))
                        {
                            ...
                            {
                                // Используем DeepSkyBlue цвет для всех нот
                                ...
                                using (SolidBrush noteBrush = new SolidBrush(noteColor))
                                {
                                    ...
                                    if (duration > 1)
                                    {
                                        ...
                                        using (Pen durationPen = new Pen(Color.DeepSkyBlue, 2))
                                        {
        ...
        public void SetChords(List<List<int>> chords, List<int> durations)
        {
            // Преобразуем список аккордов в формат, понятный компоненту
            List<int> flatNotes = new List<int>();
            List<int> flatDurations = new List<int>();

            // Предполагаем, что у нас один канал отображения
            for (int i = 0; i < chords.Count; i++)
            {
                ...
                if (chords[i].Count > 0)
                {
                    // Добавляем все ноты аккорда на одну позицию
                    foreach (var note in chords[i])
                    {
                        flatNotes.Add(note);
                        flatDurations.Add(duration);

                        // Если это не последняя нота в аккорде, добавляем маркер "продолжение аккорда"
                        if (note != chords[i].Last())
                        {
                            flatNotes.Add(-1);
                            flatDurations.Add(0); // Длительность маркера не важна
                        }
                    }
                }
                else
                {
                    // Если нот нет, добавляем 0 (паузу)
                    flatNotes.Add(0);
                    flatDurations.Add(duration);
                }
            }

            // Теперь вызываем оригинальный метод с преобразованными данными
            SetNotesWithDurations(flatNotes, flatDurations);
        }
