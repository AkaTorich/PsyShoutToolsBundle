...
{
    /// <summary>
    /// Точка входа для VST плагина BASSGenerator
    /// </summary>
    public sealed class PluginCommandStub : StdPluginCommandStub
    {
        ...
        private static Assembly OnAssemblyResolve(object sender, ResolveEventArgs args)
        {
            try
            {
                ...
                foreach (string dllName in embeddedDlls)
                {
                    ...
                    {
                        // Пытаемся загрузить из встроенных ресурсов (они встроены как BASSGeneratorVST.bin.Release.{dllName}.dll)
                        string resourceName = $"BASSGeneratorVST.bin.Release.{dllName}.dll";
        ...
        {
            return new BASSGeneratorVstPlugin();
        }
    }
}
