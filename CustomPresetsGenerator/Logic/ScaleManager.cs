        ...
        private static readonly Dictionary<string, Dictionary<string, List<int>>> BigScalePatterns = new Dictionary<string, Dictionary<string, List<int>>>
{
    ...
    "Микротональные лады", new Dictionary<string, List<int>>
    {
        ...
        /// <summary>
        /// Получить все доступные категории шкал
        /// </summary>
        public static List<string> GetCategories()
        {
            return BigScalePatterns.Keys.ToList();
        }

        /// <summary>
        /// Получить все шкалы в указанной категории
        /// </summary>
        public static List<string> GetScalesInCategory(string category)
        {
            if (BigScalePatterns.TryGetValue(category, out var scales))
            {
                return scales.Keys.ToList();
            }
            return new List<string>();
        }

        /// <summary>
        /// Получить случайную шкалу из указанной категории
        /// </summary>
        public static string GetRandomScaleFromCategory(string category, Random random)
        {
            var scales = GetScalesInCategory(category);
            if (scales.Count == 0) return null;
            return scales[random.Next(scales.Count)];
        }

        /// <summary>
        /// Получить случайную шкалу из всех категорий
        /// </summary>
        public static (string category, string scaleName) GetRandomScale(Random random)
        {
            var categories = GetCategories();
            var category = categories[random.Next(categories.Count)];
            var scaleName = GetRandomScaleFromCategory(category, random);
            return (category, scaleName);
        }

        /// <summary>
        /// Конвертировать интервалы шкалы в полутоны (совместимость с существующим кодом)
        /// </summary>
        public static int[] ConvertToSemitones(List<int> intervals)
        {
            var semitones = new List<int> { 0 }; // Начинаем с тоники
            int current = 0;
            
            foreach (var interval in intervals)
            {
                current += interval;
                semitones.Add(current);
            }
            
            return semitones.ToArray();
        }

        /// <summary>
        /// Получить шкалу в формате полутонов
        /// </summary>
        public static int[] GetScaleAsSemitones(string category, string scaleName)
        {
            if (BigScalePatterns.TryGetValue(category, out var categoryScales) &&
                categoryScales.TryGetValue(scaleName, out var intervals))
            {
                return ConvertToSemitones(intervals);
            }
            return null;
        }

        /// <summary>
        /// Получить случайную шкалу в формате полутонов
        /// </summary>
        public static int[] GetRandomScaleAsSemitones(Random random)
        {
            var (category, scaleName) = GetRandomScale(random);
            return GetScaleAsSemitones(category, scaleName);
        }

        /// <summary>
        /// Получить случайную шкалу из указанной категории в формате полутонов
        /// </summary>
        public static int[] GetRandomScaleFromCategoryAsSemitones(string category, Random random)
        {
            var scaleName = GetRandomScaleFromCategory(category, random);
            if (scaleName == null) return null;
            return GetScaleAsSemitones(category, scaleName);
        }

        /// <summary>
        /// Получить информацию о шкале (категория и название)
        /// </summary>
        public static (string category, string scaleName) GetScaleInfo(int[] semitones)
        {
            foreach (var category in BigScalePatterns.Keys)
            {
                foreach (var scale in BigScalePatterns[category])
                {
                    var scaleSemitones = ConvertToSemitones(scale.Value);
                    if (scaleSemitones.SequenceEqual(semitones))
                    {
                        return (category, scale.Key);
                    }
                }
            }
            return (null, null);
